...
public interface BusRepository extends JpaRepository<Bus, Long> {
    ...
    SELECT 
        ...
        (SELECT MAX(B.TIMESTAMP) 
         ...
         WHERE A.PLATE_NO = B.PLATE_NO) AS SEARCH_TIMESTAMP,
        (SELECT B.LOCAL_Y 
         FROM bus_produce_topic B 
         WHERE A.id = B.id LIMIT 1) AS LOCAL_Y,
        (SELECT B.LOCAL_X 
         FROM bus_produce_topic B 
         WHERE A.id = B.id LIMIT 1) AS LOCAL_X,
        (SELECT B.CURR_STATION_NM 
         FROM bus_produce_topic B 
         WHERE A.id = B.id LIMIT 1) AS CURR_STATION,
        (SELECT B.ROUTE_NUM 
         FROM bus_produce_topic B 
         WHERE A.id = B.id LIMIT 1) AS ROUTE_NUM
    FROM bus_produce_topic A
    WHERE A.TIMESTAMP >= NOW() - INTERVAL 10 SECOND
    GROUP BY A.PLATE_NO
    """, nativeQuery = true)
    List<?> findBusDetails();
}