...
public interface BusRepository extends JpaRepository<Bus, Long> {
    ...
    SELECT
        ...
        MAX(A.id) AS MAX_ID,
        (SELECT MAX(B.TIMESTAMP) FROM bus_search_topic B WHERE A.PLATE_NO = B.PLATE_NO) AS SEARCH_TIMESTAMP,
        (SELECT B.LOCAL_Y FROM bus_produce_topic B WHERE A.id = B.id LIMIT 1) AS LOCAL_Y,
        (SELECT B.LOCAL_X FROM bus_produce_topic B WHERE A.id = B.id LIMIT 1) AS LOCAL_X,
        (SELECT B.CURR_STATION_NM FROM bus_produce_topic B WHERE A.id = B.id LIMIT 1) AS CURR_STATION,
        (SELECT B.ROUTE_NUM FROM bus_produce_topic B WHERE A.id = B.id LIMIT 1) AS ROUTE_NUM
