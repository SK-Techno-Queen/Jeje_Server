...
public interface BusRepository extends JpaRepository<Bus, Long> {
    @Query(value = "SELECT b.* FROM jeju_test_topic3 b INNER JOIN (SELECT b2.plate_No, MAX(b2.timestamp) AS latestTimestamp FROM jeju_test_topic3 b2 GROUP BY b2.plate_No) latest ON b.plate_No = latest.plate_No AND b.timestamp = latest.latestTimestamp", nativeQuery = true)
